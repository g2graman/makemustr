...
install_box:
	...
	vagrant exec git clone https://github.com/g2graman/Mustr.git

run_box:
	vagrant exec cd Mustr &&
	vagrant exec sudo make install && # sudo require for -g flag of npm
	vagrant exec sudo make run && # this sudo not necessary
	vagrant ssh
...
.PHONY: check clean clean_box silent test install install_box run load_box run_box init_box
